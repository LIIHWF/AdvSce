...
from advsce.simulator import common
from typing import Dict, Type, Union, overload

from lgsvl import agent
import advsce.simulator
import advsce.map as map
from advsce.utils import DictObj, DispatchMethod, check_type
from advsce.geometry import Vector, Radian
from advsce.controller import ControllerFactory
import json, copy
...
class AgentDescription:
    ...
    def __init__(self, id:str, agent_type, model_name:str, init_state, controller, config=None):
        ...
        if type(agent_type) == str:
            self.agent_type:advsce.simulator.AgentType = advsce.simulator.AgentType.__members__[agent_type]
        elif type(agent_type) == advsce.simulator.AgentType:
            self.agent_type:advsce.simulator.AgentType = agent_type
        else:
    ...
    def _(self, config:dict):
        self.id = config['id']
        self.agent_type = advsce.simulator.AgentType.__members__[config['agent_type']]
        self.model_name = config['model_name']
        self.init_state = advsce.simulator.AgentState(config['init_state'])
...
class StaticItemDescription:
    ...
    def __init__(self, id, item_type, model_name, init_state, config=None):
        ...
        if isinstance(item_type, str):
            self.item_type = advsce.simulator.StaticItemType.__members__[item_type]
        elif isinstance(item_type, advsce.simulator.AgentType):
    ...
    def _(self, config:dict):
        self.id = config['id']
        self.item_type = advsce.simulator.StaticItemType.__members__[config['item_type']]
        self.model_name = config['model_name']
        self.init_state = advsce.simulator.ObjectState(config['init_state'])
        self.config = config['config']
...
class ScenarioDescription:
    ...
    def __init__(self, simulator, map_name:str, duration=20, simulator_config=None, weather=None, time_of_day=12, agents=None, static_items=None, traffic_lights_control=None, record_traffic_lights=None, time_step=0.1, aut_id='aut'):
        ...
        if type(simulator) == str:
            self.simulator:advsce.simulator.SimulatorType = advsce.simulator.SimulatorType.__members__[simulator]
        elif type(simulator) == advsce.simulator.SimulatorType:
            self.simulator:advsce.simulator.SimulatorType = simulator
        else:
    ...
    def _parse_from_config(self, config:dict):
        ...
        self.simulator = advsce.simulator.SimulatorType.__members__[config['simulator']]
    ...
    def _get_simulator(self):
        if self.simulator == advsce.simulator.SimulatorType.SVL:
            return advsce.simulator.SVLSimulator(**self.simulator_config)
        else:
            raise NotImplementedError

    def _get_simulator_configuration(self):
        if self.simulator == advsce.simulator.SimulatorType.SVL:
            return advsce.simulator.svl_configuration
        else:
            raise NotImplementedError
    ...
    def available(self):
        ...
        for a1 in agent_list:
            for a2 in agent_list:
                if not a1 is a2:
                    if (
                        a1.agent_type == advsce.simulator.common.AgentType.PEDESTRIAN and \
                        a2.agent_type == advsce.simulator.common.AgentType.PEDESTRIAN 
                    ):
                        continue
                    try:
