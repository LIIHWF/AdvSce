...
# from advsce.map import transform
from collections import namedtuple
from pickle import REDUCE
import lgsvl, os, copy

from advsce.simulator.common import SimulatorBase, AgentType, AgentState, AgentBase, SimulatorType, CollisionException, StaticItemBase, StaticItemType, TrafficLightState
from enum import Enum
from advsce.geometry import Vector, Radian
from advsce.utils import check_type
from advsce.controller import ApolloController, LaneFollowController, ControllerBase
import advsce.map
import json, math, logging, time
from loguru import logger
from advsce.config import LIBDIR
...
class SVLAgent(AgentBase):
    ...
    def _apply_apollo_controller(self):
        ...
        transform = advsce.map.SVLTransform(self.simulator.svl_sim)
...
class SVLSimulator(SimulatorBase):
    ...
    def set_traffic_light(self, traffic_light, init_light_state, trigger_distance, yellow_time=1):
        sim_traffic_light = self._get_sim_traffic_light(traffic_light)
        ...
        if init_light_state == TrafficLightState.GREEN:
            sim_traffic_light.control(f'green=1;trigger={trigger_distance};yellow={yellow_time};red=5;green=2;loop')
        elif init_light_state == TrafficLightState.RED:
            sim_traffic_light.control(f'red=1;trigger={trigger_distance};red={yellow_time};green=3;red=4;loop')
        else:
        ...
        if not self.svl_sim.current_scene is None:
            ...
            for controllable in self.svl_sim.get_controllables():
                if controllable.type == 'signal':
                    if controllable.control_policy[0]['value'] == 'green':
                        controllable.control('green=3;yellow=1;red=5;loop')
                    else:
                        controllable.control('red=5;green=3;yellow=1;loop')

    def run(self, time_limit=0.0, time_scale=None):
        self.svl_sim.run(time_limit=time_limit, time_scale=time_scale)

    def load_map(self, map_name):
        scene_name = advsce.map.map_config.sim_name(SimulatorType.SVL, map_name)
